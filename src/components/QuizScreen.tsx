...
export const QuizScreen: React.FC<QuizScreenProps> = ({
  ...
  savedAnswers,
  initialIndex = 0,
  initialTimeRemaining = 1800, // 30 minutes in seconds
  onAnswerChange,
  onSubmitExam,
  onTimeTick,
  onBackToMenu,
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          // Auto submit when time runs out! Keep already chosen answers intact
  ...
  const progressPercent = Math.round(((currentIndex + 1) / totalQuestions) * 100 * 10) / 10;
  ...
  return (
    <div className="mx-auto min-h-[calc(100vh-3.5rem)] max-w-4xl px-4 py-4 sm:px-6 sm:py-6">
      {/* Quiz Top Status Bar */}
      <div className="mb-4 rounded-2xl border border-zinc-200/90 bg-white p-4 shadow-2xs dark:border-zinc-800 dark:bg-zinc-900/90 transition-all">
        <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
          ...
          <div className="flex items-center gap-3 font-mono">
            <span className="rounded-md bg-zinc-100 px-2.5 py-1 text-xs font-semibold text-zinc-800 dark:bg-zinc-800 dark:text-zinc-200">
              Soal {currentIndex + 1} dari {totalQuestions}
            </span>
            <div
              className={`flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-semibold tracking-wider ${
                isTimeCritical
        ...
        {/* Progress Bar */}
        <div className="mt-3">
          <div className="flex items-center justify-between text-[11px] font-mono text-zinc-700 dark:text-zinc-300 mb-1">
      ...
      <div className="rounded-2xl border border-zinc-200/90 bg-white p-5 sm:p-7 shadow-xs dark:border-zinc-800 dark:bg-zinc-900/90 transition-all">
          ...
          {currentQuestion.options.map((optionText, optIdx) => {
            ...
            return (
              <button
                ...
                className={`group flex w-full items-start gap-3.5 rounded-xl border p-3.5 text-left transition-all duration-150 sm:items-center ${
                  isSelected
        ...
        {/* Reset Choice Action (if selected) */}
        {selectedOption !== undefined && (
          <div className="mt-3 flex justify-end">
            <button
              type="button"
              onClick={handleClearOption}
              className="inline-flex items-center gap-1 text-[11px] text-zinc-600 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-200"
            >
      ...
      {/* Bottom Navigation Buttons (Fixed/Padded for touch accessibility) */}
        ...
        <div className="flex items-center gap-2">
          {currentIndex < totalQuestions - 1 ? (
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-xs font-semibold text-zinc-800 hover:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:bg-zinc-800 active:scale-[0.98] transition-all"
            >
